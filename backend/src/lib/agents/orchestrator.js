...
const finance_agent_1 = require("./finance-agent");
const recovery_agent_1 = require("./recovery-agent");
const risk_agent_1 = require("./risk-agent");
const growth_agent_1 = require("./growth-agent");
const guardrails_1 = require("@/lib/policy/guardrails");
class MasterOrchestrator {
    static async runTask(userQuery) {
        ...
        const [financeData, recoveryData, riskData, growthData] = await Promise.all([
            finance_agent_1.FinanceAgent.getFinanceSummary(),
            recovery_agent_1.RecoveryAgent.getRecoverySummary(),
            risk_agent_1.RiskAgent.getRiskSummary(),
            growth_agent_1.GrowthAgent.getGrowthSummary(),
        ]);
        ...
        const proposedActions = proposedRawActions.map((raw) => {
            const evaluation = guardrails_1.PolicyEngine.evaluate({
