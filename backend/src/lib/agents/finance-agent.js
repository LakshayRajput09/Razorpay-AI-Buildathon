...
class FinanceAgent {
    ...
    static async getCashPosition() {
        const latest = await prisma_1.default.cashFlow.findFirst({
        ...
        const upcomingExpenses = await prisma_1.default.expense.findMany();
        return {
    ...
    static async getFinanceSummary() {
        const historicalOnly = await prisma_1.default.cashFlow.findMany({
            where: { forecasted: false },
            orderBy: { date: "desc" },
            take: 14,
        });
